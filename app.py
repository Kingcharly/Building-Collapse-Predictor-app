...
def load_model_and_artifacts():
    try:
        ...
        # Load training sample (using os.path.join to avoid path issues)
        ...
        feature_names_transformed = np.load(os.path.join(current_dir, "artifacts", "feature_names_transformed.npy"))
...
def main():
    ...
    with col1:
        ...
        input_data['collapse_risk_score'] = st.slider("Collapse Risk Score", 0.0, 10.0, 5.0, 0.1)
        input_data['column_fck'] = st.number_input("Column Compressive Strength (MPa)", 15.0, 50.0, 25.0, 1.0)
        input_data['beam_fck'] = st.number_input("Beam Compressive Strength (MPa)", 15.0, 50.0, 25.0, 1.0)
        input_data['slab_fck'] = st.number_input("Slab Compressive Strength (MPa)", 15.0, 50.0, 25.0, 1.0)
        input_data['bearing_capacity'] = st.number_input("Soil Bearing Capacity (kN/m²)", 50.0, 500.0, 150.0, 10.0)
    
    with col2:
        input_data['Y6_fyk'] = st.number_input("6mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
        input_data['Y8_fyk'] = st.number_input("8mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
        input_data['Y10_fyk'] = st.number_input("10mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
        input_data['Y12_fyk'] = st.number_input("12mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
        input_data['Y16_fyk'] = st.number_input("16mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
        input_data['Y20_fyk'] = st.number_input("20mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
        input_data['Y25_fyk'] = st.number_input("25mm Bar Yield Strength (MPa)", 200.0, 600.0, 415.0, 5.0)
